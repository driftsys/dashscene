...
// them for the host's log through `UnrecordedCounters`.
//
...
// carries no `Render Thread`, and one that draws no Canvas carries no
// `Canvas.SendWillRenderCanvases`. `unity/render-gate` is both, and carries
// three of the five: `Canvas.BuildBatch` registers there even with no Canvas in
// the scene, and `GC Allocated In Frame` registers because that player is built
// with `BuildOptions.Development`.
//
...
namespace Driftsys.Dashscene
{
    ...
    public sealed class DashsceneThreadCost : System.IDisposable
    {
        ...
        ///
        /// **Named `UnrecordedCounters` and not `Unrecorded`**, because
        /// `ThreadCostSample.Unrecorded` is the em dash a term with no reading
        /// prints — two public members of one namespace, and the two meanings
        /// are a sentence apart.
        ///
        ...
        public string UnrecordedCounters { get; } = string.Empty;
        ...
        public DashsceneThreadCost(string[] args)
        {
            ...
            UnrecordedCounters = Missing();
            if (!_main.Valid)
            {
                Reason = "the Main Thread counter is not recordable on this player, so "
                    + "there is no term to report at all. Unrecordable here: "
                    + UnrecordedCounters;
                Dispose();
                return;
            }
        ...
        /// Which counters did not start, named for `UnrecordedCounters` and `Reason`.
        ///
