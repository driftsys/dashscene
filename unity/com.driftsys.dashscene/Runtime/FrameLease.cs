...
namespace Driftsys.Dashscene
{
    ...
    public sealed class FrameLease : IDisposable
    {
        ...
        public void Dispose()
        {
            ...
            // **Latched only after the library has released.** Latching first
            // makes a failed release unrecoverable: `ReleaseLease` correctly
            // leaves the runtime's `_lease` non-null and rethrows, but this
            // object would refuse to retry, so `DashsceneRuntime.Dispose`'s
            // release branch becomes a permanent no-op, `ds_runtime_free`
            // answers `DsStatus.FrameLeased` forever and the runtime cannot be
            // freed. That defeats the ordering fix in `ReleaseLease` one layer
            // up, which is where this was found.
        ...
        //
        // **Several row sizes are shared between arrays**, so this table's
        // ORDER is load-bearing and the length check below cannot see a
        // permutation: two same-sized entries exchanged here match every stride
        // and read the wrong rows. `unity/ffi-check` asserts the names in order
        // against a literal list, which is what catches that — an enumeration
        // of which sizes collide lived here and was wrong.
        //
