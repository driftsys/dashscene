...
namespace Driftsys.Dashscene
{
    ...
    public static class ThreadCostMath
    {
        ...
        /// The value at a percentile, by `DashsceneFrameCost.At`'s index
        /// arithmetic.
        ///
        /// `values[round((len - 1) * p)]` over a sorted copy, rounding **away
        ...
        public static double At(double[] values, double percentile)
        {
            var sorted = (double[])values.Clone();
            System.Array.Sort(sorted);
            var index = (int)System.Math.Round(
                (sorted.Length - 1) * percentile, System.MidpointRounding.AwayFromZero);
            return sorted[index];
        }

        /// <summary>The sample's median, which is <see cref="At"/> at 0.5.</summary>
        public static double P50(double[] values)
        {
            return At(values, 0.5);
        }

        /// <summary>The sample's 95th percentile, which is <see cref="At"/> at 0.95.</summary>
        public static double P95(double[] values)
        {
            return At(values, 0.95);
        }

        /// The sample's largest value, which is <see cref="At"/> at 1.0.
        ///
        /// **Expressed as a percentile rather than as a loop**, because
        /// `DashsceneFrameCost` reports its own `max` that way and D1 of
        /// `docs/decisions/the-unity-painter-is-measured-against-a-faithful-canvas.md`
        /// asks for the two lines' four statistics to be read side by side.
        public static double Max(double[] values)
        {
            return At(values, 1.0);
        }
