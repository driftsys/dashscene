...
/// imported at all.
///
/// **Deliberately conditional**, and no longer vacuous. It was written while the
/// package shipped zero `.meta` files, when an unconditional form could not have
/// landed green. Story #1121 committed thirteen — generated by a Unity editor
/// rather than written by hand, which is why the partial-adoption case it was
/// aimed at did not arise — so the conditional now demands all of them.
///
/// **What it still does not check**: that any are shipped at all. Deleting every
/// `.meta` returns through the escape below and passes, and the comparison is
/// over filenames, so replacing them with empty files would pass too even though
/// the guid inside each is the load-bearing part.
#[test]
