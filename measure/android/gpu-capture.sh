...
cp "${here}/${config}" "${out}/${config}"
{
    ...
    echo "differ between Adreno, Mali and PowerVR, and the adapter is not known"
    echo "until \`just android-probe\` reports it on the device. Add them in a"
    echo "second pass, once the probe has named the GPU:"
    echo
    echo "    adb shell perfetto --query | head -60"
    echo
    echo "lists the data sources that device actually has, including"
    echo "\`gpu.counters\` and its available counter ids when the vendor exposes"
    echo "them."
