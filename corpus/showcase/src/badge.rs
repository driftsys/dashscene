...
//! scene injects. Only `crate::resources::solver` carries a typesetter and
//! the atlas list, so a label authored by the host — which holds no solver
//! handle — would commit through a text-incapable path and stage no glyph
//! runs at all. It lives here for the same reason the
