...
use dashpack::profile::{
    self, AssetClass, BANDS, Binding, Contract, Derivation, HIFI_IMAGE_FILL, LITE_IMAGE_FILL,
...
/// almost exactly at every footprint. It exists to make the `lite-image-fill`
...
/// either side of Lite's 5 % budget across the ladder, which is what makes the
...
///   HiFi and at 12x12 under Lite, and `detail-noise` ships uncompressed under
///   HiFi and at 6x6 under Lite. A profile pair that chose the same rung
...
fn hifi_and_lite_choose_different_rungs_for_the_same_asset() {
    let differing: Vec<&str> = FIXTURES
        ...
        .filter(|fixture| {
            ...
            rung(Profile::HiFi) != rung(Profile::Lite)
        })
        .map(|fixture| fixture.name)
        .collect();
    assert!(
        differing.len() >= 2,
        "only {differing:?} separate HiFi from Lite; a profile pair that agrees everywhere is \
...
const _: () = assert!(
    LITE_IMAGE_FILL.channel_delta > HIFI_IMAGE_FILL.channel_delta,
    "Lite's per-texel threshold must be looser than HiFi's"
);
const _: () = assert!(
    LITE_IMAGE_FILL.differing_fraction > HIFI_IMAGE_FILL.differing_fraction,
    "Lite's area budget must be looser than HiFi's"
);
