...
/// because no asset in the tree separates the two profiles' *area budgets*: the
/// real image fills are a gradient and flat rectangles, which ASTC reproduces
/// almost exactly at every footprint. It exists to make the `lofi-image-fill`
/// budget the binding term, which is the property #422 found the render bands
/// lacked. It is deterministic — an integer hash, no floating point, no
/// randomness — so it is as reproducible as a committed file.
