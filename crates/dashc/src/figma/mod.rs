...
impl Walk<'_> {
    ...
    /// page-absolute box into the parent-relative intent `Scd` wants. `None`
    ...
    fn visit(
        ...
        // Scd has no field for a hidden node, and no way to represent one
        ...
        // Scd carries no opacity vocabulary — no Construct fits, so opacity
        ...
        // Scd carries no rotation vocabulary — no Construct fits, so a
        ...
        // Scd carries no mask vocabulary — no Construct fits, so a mask node
    ...
    fn fill_of(&mut self, node: &Node, path: &str) -> Result<Option<PaintKind>, CompileError> {
        ...
        if visible.next().is_some() {
            ...
            // array. Stacking is an Scd expressiveness gap (debt #146), not
