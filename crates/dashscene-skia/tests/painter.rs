...
// ---------------------------------------------------------------------------
// A coverage mask that draws nothing (issue #1000)
// ---------------------------------------------------------------------------
...
/// divides `dest.width()` by `aw` and `dest.height()` by `ah`, so a fixture
/// zeroing only one could not tell a guard over the pair from a guard over that
...
const DRAWS_NOTHING: [(&str, [u32; 4], [f32; 4]); 15] = [
    ...
    ("a NaN left bound", [0, 0, 8, 8], [f32::NAN, 0.0, 8.0, 8.0]),
    ("a NaN top bound", [0, 0, 8, 8], [0.0, f32::NAN, 8.0, 8.0]),
    ("a NaN right bound", [0, 0, 8, 8], [0.0, 0.0, f32::NAN, 8.0]),
    (
        "a NaN bottom bound",
        [0, 0, 8, 8],
        [0.0, 0.0, 8.0, f32::NAN],
...
/// A scene of one 8x8 node whose fill is masked by `field`, over an 8x8 atlas.
fn masked_fill_scene(field: dashpaint::VectorField) -> (Vec<RectEntry>, PaintTable, ImageTable) {
    let mut images = ImageTable::new();
    let image = images.push(ImageAsset {
        format: dashpaint::ImageFormat::Png,
        bytes: solid_atlas_png(8),
    });
    let mut paints = PaintTable::new();
    let fill = paints.intern_fill(&FillSpec::Solid { color: RED });
    let entry = paints.push_with(
        ...
        EntryParts {
            shape: Some(dashpaint::VectorField { image, ..field }),
            ..EntryParts::default()
        },
    );
    (vec![field_node(entry)], paints, images)
}

/// A scene of an opaque white bar over the left half, then a frosted node over
/// the whole surface whose backdrop blur is confined to `field` — or, for
/// `None`, the bar alone.
///
/// The bar is what makes the backdrop path measurable: a blur has no output of
/// its own, so a frosted node over an empty surface renders the same whether
/// the blur ran or not.
fn backdrop_scene(field: Option<dashpaint::VectorField>) -> Vec<u8> {
    backdrop_scene_at(field, 0.0)
}

/// [`backdrop_scene`] with the frosted node's own origin moved to `origin` on
/// both axes, so a caller can degenerate `dest` through the node rather than
/// through the field.
fn backdrop_scene_at(field: Option<dashpaint::VectorField>, origin: f32) -> Vec<u8> {
    let mut images = ImageTable::new();
    let image = images.push(ImageAsset {
        format: dashpaint::ImageFormat::Png,
        bytes: solid_atlas_png(8),
...
/// The 8x8 node both field scenes place their mask on.
fn field_node(paint: PaintIndex) -> RectEntry {
    RectEntry {
        x: 0.0,
        y: 0.0,
        w: 8.0,
...
/// A sound field draws, so the two tests below refuse what has no area rather
/// than refusing every field (issue #1000).
///
/// Without this the whole pair passes with `field_coverage` returning `None`
/// unconditionally, which is the mutation the two `assert_eq!`s against a
/// draws-nothing surface cannot see on their own.
#[test]
fn a_sound_coverage_mask_still_draws() {
    ...
    let (rects, paints, images) = masked_fill_scene(field);
    let rgba = render(&rects, &paints, &images, 8);
    assert_eq!(
        px(&rgba, 8, 4, 4),
        RED_RGBA,
        "the atlas is solid white, so the whole quad resolves as inside the shape"
    );
    assert_ne!(
        backdrop_scene(Some(field)),
        backdrop_scene(None),
        "a sound field's backdrop blur must change the bar under it, or the backdrop assertions \
         below compare two pictures that were never going to differ"
    );
}

/// A field naming no area draws nothing through the masked-fill path, which is
/// the answer `dashscene-gpu` reaches by skipping it (issue #1000).
///
/// This path already produced these pixels before the guard, by accident: the
/// infinite `sx` reached the resolve shader, the shader answered zero coverage,
/// and the `DstIn` mask erased the fill inside the saved layer. It is pinned
/// because the picture must not depend on that — a shader whose behaviour on an
/// infinity is not stated is not a guarantee, and the backdrop path below is
/// what the same infinity does when nothing swallows it.
#[test]
fn a_coverage_mask_with_no_area_draws_nothing_through_the_fill() {
    let empty = {
        let mut paints = PaintTable::new();
        paints.push(PaintEntry::default());
        render(&[field_node(PaintIndex(0))], &paints, &ImageTable::new(), 8)
    };
    for (name, atlas_rect, plane_bounds) in DRAWS_NOTHING {
        let (rects, paints, images) = masked_fill_scene(dashpaint::VectorField {
            image: 0,
            atlas_rect,
            plane_bounds,
            distance_range: 4.0,
...
/// The same field draws nothing through the **backdrop-blur** path, and this is
/// the half that was a live divergence (issue #1000).
///
/// `draw_backdrop_blur_field` clips to the coverage shader and opens a layer
/// with a backdrop filter. With an infinity in the shader's local matrix the
/// clip did not confine anything and the layer composited over the backdrop and
/// **erased** it: measured at 32 of these 64 pixels, the whole bar, from opaque
/// white to transparent — the reference painter destroying content the lean
/// painter leaves untouched.
///
/// The comparison is against the same scene with the frosted node removed
/// rather than against a constant, so it states the property that matters: a
/// field that draws nothing leaves the frame exactly as a scene without it.
#[test]
fn a_coverage_mask_with_no_area_draws_nothing_through_the_backdrop() {
    let without = backdrop_scene(None);
    for (name, atlas_rect, plane_bounds) in DRAWS_NOTHING {
        assert_eq!(
            backdrop_scene(Some(dashpaint::VectorField {
                image: 0,
                atlas_rect,
                plane_bounds,
                distance_range: 4.0,
            ...
            without,
            "a frosted node whose field has {name} must leave the frame as a scene without that \
             node, not erase what is under it"
        );
    }
}
...
/// nothing for a NaN device quad on both paths — so reverting the guard does
/// not fail this test. It is not the erasure of issue #1000, which needed a
/// finite quad and an infinite scale. What the test states is that the frame
/// does not depend on which mechanism refuses the node, which is the property
...
#[test]
fn a_frosted_node_with_an_out_of_domain_origin_draws_nothing() {
    ...
    for (name, origin) in [
        ...
        assert_eq!(
            ...
            without,
            "a frosted node at {name} must leave the frame as a scene without that node, not \
