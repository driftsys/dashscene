...
#1443 raised the sweep to, per sweep: 2 **frame-cost** reports on `surfaces`
(2/2/2 across A, B and C), 5 on `typography` (5/5/4) and 5 on `layout` (5/5/5).
"The thread-time line, and the URP floor" below carries that run, and its table
counts **thread-time** windows rather than these. The two lines close windows of
the same 240 frames, offset by the 60 the thread-time line discards at each
entry change — so a dwell yields one fewer thread window than frame-cost window
per entry, except where the entry is slow enough that only one of each fits. So
a re-run of this sweep is not one sample per entry. The lean painter's table
below discounts its own first sample as pipeline warm-up; nothing here can, so
`max` in particular carries warm-up. That is what the `typography` spread of
0.45 to 0.76 is.
...
`draw` mean of **0.20-0.22 ms** and the thread-time line reports a main-thread
...
s per entry, host clock `20260905T215509Z`, from the branch of PR #1456. The
sweep script stamps its own `git rev-parse` into the captures on the shelf —
`3064311` there — and that commit does not survive the branch's squash. The
commits after it change this record, the fixtures, the harness test and
`frame-table.py`, which reads the captures rather than producing them; none of
them changes code the player runs.

    entry                       mean         p50          p95          max          cpu %   n
    scene surfaces              32.76-32.83  33.38-33.42  34.16-34.67  47.65-50.90  25-27    6
    scene typography            16.68-16.76  16.68-16.72  17.35-17.68  18.54-29.28  44-57   12
    scene layout                16.68-16.69  16.68-16.72  17.17-17.47  18.51-23.94  47-49   12
    paint: fills, strokes, …    16.68-16.69  16.67-16.71  17.39-17.53  18.93-22.64  45-49   12
    layout: a variant set, …    16.68-16.69  16.66-16.71  17.35-17.63  18.25-20.84  46-49   12
    layout: the variant shelf   16.68-16.69  16.66-16.72  17.30-17.61  18.34-22.69  47-49   12

All four statistics are the main-thread term, in milliseconds. D1 states its
at-rest test against the floor's own p95 and asks the reading to state all four
for both renderers, which is why the instrument reports the same four
`FrameCostSample` does rather than a mean and a p95.
...
line reports what a frame allocates. It reads **832 B/frame** on the shelf's
`render-gate-report.txt`, after the accumulator stopped composing its sampling
key as `entry + "@" + width + "x" + height` once per push; the run before that
change read 1024, and that run's report was overwritten rather than kept. The
192-byte difference is the instrument's own allocation, and it does not rest on
the missing report: `unity/ffi-check` measures the same key directly with
`GC.GetAllocatedBytesForCurrentThread` — 192 B per push for the gate's own key
and 200 for the device's longer entry name. It mattered because D3 states the
zero-allocation rule over `GC.GetAllocatedBytesForCurrentThread` around the
host's `Update` **in this gate**, which is story #1445's measurement — an
instrument that allocated would have been measuring itself. `unity/ffi-check`
...
which reproduces that HDR-off figure to within 0.1 fps, across two days and one
commit range (`5b279f6` on 2026-09-03, this story's build on 2026-09-05). That
is corroboration across two builds, not across time. No separate "before" APK
was built for this story: the field's effect was already measured on this
device, and the other four fields were measured to be at their defaults by the
line quoted above.
