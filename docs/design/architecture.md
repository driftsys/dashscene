...
| `crates/dashscene-web`        | web integration — canvas-to-surface handoff, the `requestAnimationFrame` loop, resize rebuild, byte-range `.dsb` load                                                 | built at v0.17 (story #741); the wasm/tiny-skia painter the name once described is retired, superseded by `dashscene-gpu` |
| `crates/dashscene-desktop`    | desktop integration — window-to-surface handoff, the `winit` frame loop, resize rebuild, the published `Present` seam, a mapped `.dsb` load bounded by the shown root | built at v0.17 (story #794); `demo` keeps the demonstration and consumes it                                               |
...
crate at v0.17, which is a host concern rather than a painter. **Two things this does not claim**: the
