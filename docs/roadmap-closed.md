# Closed slices — v0.1 to v0.20

The full record of every v0 slice that has closed: what it delivered, what it
depended on, and the revision notes written at each close. Split out of
[`roadmap.md`](roadmap.md) so that file carries only open work; the index of
these slices, and every slice still open, lives there.

Nothing here is superseded — a closed slice's record stays normative for what it
delivered. Slices close in order, so this file only grows at a phase-end.
