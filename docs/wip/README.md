...
twelve and exits non-zero. All twelve are deliberate, accepted exceptions rather
than ungardened debt, and they are recorded here so the gate's result is
explained rather than merely tolerated.

**Ten are design captures, two are a Superpowers specification and its plan, and
no driver prompt is held.** That no prompt is held is the state this file has
been in at several earlier points — 2026-08-02, 2026-08-07 and the v0.17 close
among them — and it has held continuously since the v0.19 close (story #843,
2026-08-16), because a prompt belongs to a slice and leaves with the work it
carried. **v0.21's stories have each archived their working memory inside the
pull request that landed the work**, so none of it is in this directory's
history on `main`: story #1230's driver prompt is
...
rule gives a spent driver prompt. So the count above is twelve because twelve is
...
Not all twelve are untouched: several are **partly gardened**, their implemented
...
of them are design captures and none is a driver prompt — held until the
specification below was added. Its condition is in the table below, and it is
one commit with the file, per this file's own rule.

**Twelve, when the Canvas-parity specification and its plan were added, in one
commit** (2026-09-05). `2026-09-05-unity-painter-beside-a-faithful-canvas.md` is
the first Superpowers specification held here for an epic rather than for one
story (story #1329's pair, above, was tracked here for a story); it carries the
criterion, the fairness rules and the story shape for the v0.21 epic that
measures the Unity painter beside a faithful uGUI Canvas. Its plan,
`…-canvas-plan.md`, is one more plan in a directory that has held many —
seventeen were added here on `main` before it,
`git log origin/main
--diff-filter=A --name-only -- 'docs/wip/*-plan.md' | sort -u`
is the derivation, and the single-instance lane's above was staged here on a
branch — written for an epic's ten stories rather than for one story. Neither
holds a driver prompt. Both rows are in the table below, and the two leave
together.
...
| `2026-09-05-unity-painter-beside-a-faithful-canvas.md`      | the epic it specifies closes: the comparison table is in `docs/design/android-toolchain.md` and story #1347 is settled on it. **Partly gardened first**: story #1442 writes §2 to §4 and §9 into `docs/decisions/the-unity-painter-is-measured-against-a-faithful-canvas.md`, and in the same PR this file's `status` line names that record as the owner of those sections, so what stays here is the story shape the nine later stories read, not a copy. The rest leaves with the plan when the last story lands                                                                                                                                                                                                                                                                                                                                                                                                               |
| `2026-09-05-unity-painter-beside-a-faithful-canvas-plan.md` | the same condition as the specification above, which it implements: the two leave together when the epic's last story archives them. Until then it is the plan each story session reads, and a story that finds a symbol moved corrects its own section here rather than working around it                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
