...
measured twelve at story #1230; story #859 added the data plane's two, story
#1124 the ranged loader and story #1123 the atlas seam's two, and that
measurement is left as it was taken.) **The step that does not follow is that a
check therefore needs one.** `cargo rustc -p dashpaint-abi --crate-type cdylib`
emits a dynamic library carrying all **56** of them — two per gated type — with
no manifest change, alongside the four table functions that make **60**
`dashpaint_abi_*` exports in total; it was 58 before story #859 added
`GroupComposite`. So story #1239's `unity/abi-check` calls them directly and the
published crate stays an rlib. **Story #859 settled the rest**: they are not
re-exported through `dashscene-ffi`, whose `DsSlice::stride` reports this
build's row size per array instead.
