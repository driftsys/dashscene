...
1. Define all types in `dashpaint`, dependency-free; the runtime converts
   at the boundary.
...
- A `dashscene-core` dependency is explicitly excluded by story #3 (the
  trait must compile and be testable standalone), and it would prevent
  story A and story B from proceeding in parallel.
