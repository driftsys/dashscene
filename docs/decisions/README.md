...
- [one-solver-per-live-scene.md](one-solver-per-live-scene.md) — a retained
  Taffy tree is correct only while one solver sees every commit that consumes
  the layout-dirty set it patches from; `corpus/showcase` therefore keeps one
  `TaffySolver::owning` per scene and stages its variant switch for
  `LiveScene::tick` to publish (story #950), and `Txn::commit_with` drains that
  set only when the solver says it read it, so `dashlang`'s replayed
  contained-write commits leave the restyle they did not do owed rather than
  discarding it (issue #1148); binds #164, #191, #771 and #863.
...
- [the-gpu-frame-on-the-target-device-is-budgeted.md](the-gpu-frame-on-the-target-device-is-budgeted.md)
  — **accepted** (2026-09-04, owner ruling): the showcase scenes present at the
  panel's refresh rate at native resolution on the target device, read from the
  compositor, not from either painter's own report. On the Pixel 5 that is the
  `surfaces` scene's 31 ms GPU frame inside 16.7 ms. The route is R-T2 first
  (stories #1412 and #1293), then the per-kind cost (#1413); the instrument is
  #1296's. Pins one device and one extent, not a display class — #549 stands.
...
- [the-c-abi-runtime-handle-is-generational.md](the-c-abi-runtime-handle-is-generational.md)
  ...
  thread-affine table**, not by a pointer, and **a handle value identifies at
  most one runtime for the life of the process** — so a handle that is stale,
  forged or from another thread produces a `DsStatus` the host can act on rather
  than undefined behaviour (owner's ruling on issue #1226, 2026-08-18). It
  landed in v0.21 before story #859, while the only consumers were this
  repository's own JNI layer and its C conformance test; the Unity C# host
  followed at story #1121. The record rules those three things, **hands the
  mechanics to the implementing pull request as seven questions**, and carries
  what that implementation answered; it is neither an `unsafe` reduction nor a
  CodeQL fix.
