...
- `Result` return (option 3): rejected because no fallible operation
  ...
  painter is a broken inter-crate contract, not a runtime condition.
  `PaintTable::resolve` centralizes the panic for that case, so every
  backend fails the same way; silently skipping the rect would be the
  silent drop P4 forbids.
