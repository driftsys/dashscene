...
fiction (`docs/decisions/dashc-scd-model-and-load-path.md`). This note records
...
## Figma's boxes are page-absolute; `Scd`'s are parent-relative

`absoluteBoundingBox` is in page coordinates. `Scd`'s `Box2D` is
